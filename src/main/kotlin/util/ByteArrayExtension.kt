...
fun ByteArray.toShort(): Short =
    ByteBuffer.wrap(this)
        .getShort(0) // this.take(2).fold(0, {acc, b -> (acc shl 8) or (b.toInt())}).toShort()

fun ByteArray.toInt(): Int =
    ByteBuffer.wrap(this).getInt() // this.take(4).fold(0, {acc, b -> (acc shl 8) or (b.toInt())})

fun ByteArray.toLong(): Long =
    ByteBuffer.wrap(this).getLong() // this.take(8).fold(0, {acc, b -> (acc shl 8) or (b.toLong())})
...
fun Short.toByteArray(): ByteArray =
    ByteBuffer.allocate(Int.SIZE_BYTES).putShort(this).array() // numberToByteArray(this, 2)
// //ByteBuffer.allocateDirect(2).putShort(this).array()//byteArrayOf(((this-0xff)
// and 0xff).toByte(), (this-0xff).toByte())
// fun Short.toUByteArray(): UByteArray = numberToUByteArray(this.toULong(), 2)
// fun Short.toUByteArray(): UByteArray {
fun Int.toByteArray(): ByteArray =
    ByteBuffer.allocate(Int.SIZE_BYTES)
        .putInt(this)
        .array() // arrayOf(24, 16, 8, 0).map({i -> ((this shr i) and
// (0xff)).toByte()}).toByteArray()
// fun Int.toUByteArray(): UByteArray = this.toByteArray().map { it.toUByte() }.toUByteArray()
fun Long.toByteArray(): ByteArray =
    ByteBuffer.allocate(Long.SIZE_BYTES)
        .putLong(this)
        .array() // arrayOf(56, 48, 40, 32, 24, 16, 8, 0).map({i -> ((this shr i) and
                 // (0xff)).toByte()}).toByteArray()
...
// fun UShort.toByteArray(): ByteArray = byteArrayOf(((this - 0xffu) and 0xffu).toByte(), (this and
// 0xffu).toByte())
// fun UShort.toUByteArray(): UByteArray = this.toByteArray().map { it.toUByte() }.toUByteArray()
// fun UInt.toByteArray(): ByteArray = arrayOf(24, 16, 8, 0).map({i -> ((this shr i) and
// (0xffu)).toByte()}).toByteArray()
// fun UInt.toUByteArray(): UByteArray = arrayOf(24, 16, 8, 0).map({i -> ((this shr i) and
// (0xffu)).toUByte()}).toUByteArray()
// fun ULong.toByteArray(): ByteArray = arrayOf(56, 48, 40, 32, 24, 16, 8, 0).map({i -> ((this shr
// i) and (0xffu)).toByte()}).toByteArray()
