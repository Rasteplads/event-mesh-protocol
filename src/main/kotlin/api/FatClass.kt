...
final class FatClass<ID, Data> private constructor(
    ...
    companion object {
        ...
        interface Builder<ID, Data> {
            ...
            /**
             * Sets a function that will be called on every message that is not filtered (See [addFilterFunction]).
            ...
            /**
             * Sets the time a message ID should be saved in the cache.
             * This is to reduce the number of redundant relays, since relaying the same message several times is not needed.
             ...
             * This number denotes how many times a node can relay the message in the network.
        ...
        private class BuilderImpl<ID, Data> : Builder<ID, Data> {
            ...
            override fun build(): FatClass<ID, Data> {
                ...
                check(::intoID.isInitialized) {"Function to convert binary data into `ID` is necessary"}
                check(::intoData.isInitialized) {"Function to convert binary data into `Data` is necessary"}
                check(::fromID.isInitialized) {"Function to convert `ID` into binary data is necessary"}
                check(::fromData.isInitialized) {"Function to convert `Data` into binary data is necessary"}
                check(::msgData.isInitialized) {"Either a constant or a generator function for the advertising `Data` must be set"}
                check(::msgID.isInitialized) {"Either a constant or a generator function for the advertising `ID` must be set"}
